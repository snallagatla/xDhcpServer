...
#region Input Validation
    ...
    $null = Get-DhcpServerv6Scope -Prefix $Prefix -ErrorAction SilentlyContinue -ErrorVariable err
    if($err)
    {
        $errorMsg = $($LocalizedData.InvalidScopeIdMessage) -f $Prefix
    ...
    $ensure = 'Absent'
        try
        {
            $dhcpOption = Get-DhcpServerv6OptionValue -Prefix $Prefix
            
            if($dhcpOption)
            {
                $DomainSearchList = (($dhcpOption | Where-Object Name -like 'Domain Search List').value)[0]
                $ensure = 'Present'
                $dnsServerIP = ($dhcpOption | Where-Object Name -like 'DNS Recursive Name Server IPv6 Address List').value
...
function Test-TargetResource
{
    ...
    # Convert the ScopeID to be a valid IPAddress
    $ScopeID = (Get-ValidIpAddress -ipString $Prefix -AddressFamily $AddressFamily -parameterName 'Prefix').ToString()
    ...
    if($dhcpOption)
    {
            ...
            if($PSBoundParameters.ContainsKey('DnsServerIPAddress'))
            {
                ...
                if((-not $dnsServerIP) -or (Compare-Object $dnsServerIP $DnsServerIPAddress))
                {
                    ...
                    if($Apply)
                    {
                        ...
                        Set-DhcpServerv6OptionValue -Prefix $Prefix -DnsServer $DnsServerIPAddress -Force
            ...
            if($PSBoundParameters.ContainsKey('DomainSearchList'))
            {
                ...
                if($dnsDomainName -ne $DomainSearchList) 
                {
                    ...
                    if($Apply)
                    {
                        ...
                        Write-Verbose -Message $settingPropertyMessage
                        Set-DhcpServerv6OptionValue -Prefix $Prefix -DomainSearchList $DomainSearchList
        ...
        else
        {
            if($Apply)
            {
                ...
                {
                    Remove-DhcpServerv6OptionValue -Prefix $Prefix -OptionId $option
