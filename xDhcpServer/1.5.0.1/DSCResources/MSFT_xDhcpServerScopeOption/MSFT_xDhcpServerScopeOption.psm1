...
#region Input Validation
    ...
    $ensure = 'Absent'
        try
        {
            ...
            if($dhcpOption)
            {
                $dnsDomain = ($dhcpOption | Where-Object OptionId -Like 15).value
                $ensure = 'Present'
                $dnsServerIP = ($dhcpOption | Where-Object OptionId -Like 6).value
                ...
                $CiscoTFTP = ($dhcpOption | Where-Object OptionId -Like 150).value
...
#region Input Validation
    ...
    # Array of valid CiscoTFTP Server IP Address
    [String[]]$validCiscoTFTP = @()

    # Convert the NTPServer data to be valid IPAddress
    
    foreach ($CiscoTFTPIP in $CiscoTFTP)
    {
        $validCiscoTFTP += (Get-ValidIpAddress -ipString $CiscoTFTPIP -AddressFamily $AddressFamily -parameterName 'CiscoTFTP').ToString()
    }
    $CiscoTFTP = $validCiscoTFTP
...
function ValidateResourceProperties
{
    ...
    $dhcpOption = Get-DhcpServerv4OptionValue -ScopeID $ScopeID
    $dhcpScope = Get-DhcpServerv4Scope -ScopeID $ScopeID
    $dhcpMicrosoftOption = Get-DhcpServerv4OptionValue -VendorClass 'Microsoft Options' -ScopeId $ScopeID
    ...
    if($dhcpOption)
    {
        ...
        if($Ensure -eq 'Present')
        {
            ...
            # If CiscoTFTP is specified, test that
            if($PSBoundParameters.ContainsKey('CiscoTFTP'))
            {
                # Test the DNS Server IPs
                $checkPropertyMessage = $($LocalizedData.CheckPropertyMessage) -f 'Cisco TFTP'
                Write-Verbose -Message $checkPropertyMessage
                
                $CiscoTFTPIP = ($DhcpOption | Where-Object OptionId -eq 150).Value
                # If comparison return something, they are not equal
                if((-not $CiscoTFTPIP) -or (Compare-Object $CiscoTFTPIP $CiscoTFTP))
                {
                    $notDesiredPropertyMessage = $($LocalizedData.NotDesiredPropertyMessage) -f 'Cisco TFTP ip', ($CiscoTFTP -join ', '), ($CiscoTFTPIP -join ', ')
                    Write-Verbose -Message $notDesiredPropertyMessage
                    if($Apply)
                    {
                        $settingPropertyMessage = $($LocalizedData.SettingPropertyMessage) -f 'Cisco TFTP ip'
                        Write-Verbose -Message $settingPropertyMessage

                        #Test if Option 150 Definition exit
                        $dhcpDefnition = Get-DhcpServerv4OptionDefinition
                        $CiscoTFTPDef = ($dhcpDefnition | Where-Object OptionId -Like 150).OptionId
                        if($CiscoTFTPDef -eq $null)
                        {                      
                            Add-DhcpServerv4Class -Name 'Cisco' -Description 'Vendor Class Identifier for Cisco TFTP' -Type Vendor -Data "Cisco TFTP"
                            Add-DhcpServerv4OptionDefinition -OptionId 150 -Name 'Cisco TFTP' -Description 'Cisco TFTP' -Type IPv4Address -MultiValued

                         }

                        Set-DhcpServerv4OptionValue -ScopeId $ScopeID -OptionId 150 -Value $CiscoTFTP -Force
                        $setPropertyMessage = $($LocalizedData.SetPropertyMessage) -f 'Cisco TFTP ip', ($CiscoTFTP -join ', ')
                        Write-Verbose -Message $setPropertyMessage
                ...
                {
                    $desiredPropertyMessage = $($LocalizedData.DesiredPropertyMessage) -f 'Cisco TFTP ip'
                    Write-Verbose -Message $desiredPropertyMessage
                }
            }
    ...
    else
    {
        ...
        if($Apply)
        {
            ...
            if($Ensure -eq 'Present')
            {
                ...
                # If Cisco TFTP is specified pass it
                if($PSBoundParameters.ContainsKey('CiscoTFTP'))
                {
                    $parameters['CiscoTFTP'] = $CiscoTFTP
                }
